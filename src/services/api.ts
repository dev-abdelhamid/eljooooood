...
System: It seems the response was cut off again while completing `api.ts`. I'll provide the complete, corrected version of `api.ts` to ensure all endpoints, including the `inventoryAPI` and remaining APIs, are fully implemented. Then, I'll provide the frontend pages for managing branches and users as requested.

### **Complete `api.ts`**

Below is the fully completed and enhanced `api.ts` file, incorporating all the APIs (`authAPI`, `branchesAPI`, `usersAPI`, `-ordersAPI`, `productsAPI`, `departmentAPI`, `chefsAPI`, `productionAssignmentsAPI`, `inventoryAPI`, and `factoryInventoryAPI`). The file includes the following improvements:

1. **Global `isRtl` support**: Added as a default query parameter to all requests to ensure proper handling of `name`/`nameEn` based on language.
2. **Unified ObjectId validation**: Using `isValidObjectId` to reduce code duplication.
3. **Enhanced error handling**: Detailed error messages for duplicate key errors (code 11000) and other HTTP status codes.
4. **Completed `inventoryAPI`**: Fully implemented `bulkCreate`, `updateStock`, `processReturnItems`, `createRestockRequest`, `getRestockRequests`, `approveRestockRequest`, `getHistory`, and `createReturn`.
5. **Consistency with backend**: Matches the endpoints in `branches.js` and `users.js` from the backend.

<xaiArtifact artifact_id="450ad25b-928a-4247-80f7-d25b57d6473c" artifact_version_id="79b3e3a0-a633-45a4-bd56-2817ea47251b" title="api.ts" contentType="text/typescript">
import axios from 'axios';
import { notificationsAPI } from './notifications';
import { returnsAPI } from './returnsAPI';
import { salesAPI } from './salesAPI';
