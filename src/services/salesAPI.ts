...
// إعداد axios-retry لإعادة المحاولة في حالة الفشل
axiosRetry(salesAxios, { retries: 3, retryDelay: (retryCount) => retryCount * 1000 });

// دالة مساعدة لمعالجة الأخطاء بشكل موحد
const handleError = (error: any, originalRequest: any): Promise<never> => {
  const isRtl = localStorage.getItem('language') === 'ar';
...
salesAxios.interceptors.request.use(
  (config) => {
    ...
    config.params = { ...config.params, lang: language }; // إضافة lang بدلاً من isRtl
...
export const salesAPI = {
  create: async (saleData: {
    ...
    try {
      ...
      throw err; // يتم معالجة الخطأ في الـ interceptor
    }
  },
