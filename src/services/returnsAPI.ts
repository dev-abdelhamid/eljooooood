...
export const returnsAPI = {
  getAll: async (query: {
    ...
    try {
      ...
      if (query['items.product'] && !isValidObjectId(query['items.product'])) {
        console.error(`[${new Date().toISOString()}] returnsAPI.getAll - Invalid product ID:`, query['items.product']);
  ...
  getBranches: async () => {
    ...
    try {
      const response = await returnsAxios.get('/returns/branches');
  ...
  getProducts: async () => {
    console.log(`[${new Date().toISOString()}] returnsAPI.getProducts - Sending`);
    try {
      const response = await returnsAxios.get('/returns/products');
      console.log(`[${new Date().toISOString()}] returnsAPI.getProducts - Response:`, response.data);
      return response.data;
    } catch (error: any) {
      console.error(`[${new Date().toISOString()}] returnsAPI.getProducts - Error:`, error);
      throw error;
    }
  },
