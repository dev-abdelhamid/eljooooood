...
export const Orders: React.FC = () => {
  ...
  }, [state.filterStatus, state.filterBranch, state.searchQuery, isRtl, t, user.role]);
  ...
  const handleStatusChange = useCallback(async (orderId: string, newStatus: OrderStatus) => {
    ...
    try {
      ...
      dispatch({ type: 'SET_ERROR', payload: t('errors.status_update_failed', { message: error.message }) });
  ...
  const handleAssignChefs = useCallback(async (orderId: string) => {
    ...
    try {
      ...
      emit('taskAssigned', { orderId, items: assignments });
      fetchData();
    } catch (error: any) {
      console.error(`[${new Date().toISOString()}] Assign chefs error:`, error);
      dispatch({ type: 'SET_ERROR', payload: t('errors.assign_chefs_failed', { message: error.message }) });
  ...
  }, [state.assignFormData, t, isRtl, emit, fetchData]);
  ...
  const filteredAndSortedOrders = useMemo(() => {
    ...
    result.sort((a, b) => {
      ...
      if (state.sortBy === 'date') {
        ...
        valueA = a.priority || 'medium';
        valueB = b.priority || 'medium';
      }
    ...
    <div className={`container mx-auto p-4 sm:p-6 ${isRtl ? 'font-arabic' : 'font-sans'}`}>
      ...
      >
        ...
        <div className={`flex flex-col sm:flex-row gap-2 sm:gap-4 ${isRtl ? 'sm:flex-row-reverse' : ''}`}>
          ...
          <Button
            variant="secondary"
            onClick={() => exportToExcel(filteredAndSortedOrders, isRtl, calculateAdjustedTotal, calculateTotalQuantity, translateUnit)}
      ...
      <Card className="mb-6 p-4 sm:p-6 bg-white rounded-lg shadow-md border border-gray-100">
