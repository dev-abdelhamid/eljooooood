...
export const Orders: React.FC = () => {
  ...
  useEffect(() => {
    ...
    socket.on('disconnect', (reason) => {
      ...
      dispatch({ type: 'SET_SOCKET_CONNECTED', payload: false });
      dispatch({ type: 'SET_SOCKET_ERROR', payload: t('errors.connection_lost') });
    });
    ...
    socket.on('orderStatusUpdated', ({ orderId, status }: { orderId: string; status: Order['status'] }) => {
      ...
      toast.info(t('orders.status_updated_to', { status: t(`order_status.${status}`) }), {
        position: isRtl ? 'top-left' : 'top-right',
        autoClose: 3000,
      });
    });
    ...
    socket.on('itemStatusUpdated', ({ orderId, itemId, status }: { orderId: string; itemId: string; status: string }) => {
      ...
      toast.info(t('orders.item_status_updated_to', { status: t(`item_status.${status}`) }), {
        position: isRtl ? 'top-left' : 'top-right',
        autoClose: 3000,
      });
    });
    ...
    socket.on('returnStatusUpdated', ({ orderId, returnId, status }: { orderId: string; returnId: string; status: string }) => {
      ...
      toast.info(t('orders.return_status_updated_to', { status: t(`return_status.${status}`) }), {
        position: isRtl ? 'top-left' : 'top-right',
        autoClose: 3000,
      });
    });
  ...
  const fetchData = useCallback(
        ...
        dispatch({
          ...
          payload: chefsResponse.data
            ...
            .map((chef: any) => ({
              ...
              name: chef.user?.name || chef.name || t('users.unknown'),
              nameEn: chef.user?.nameEn || chef.nameEn,
              displayName: isRtl ? (chef.user?.name || chef.name) : (chef.user?.nameEn || chef.nameEn || chef.user?.name || chef.name || t('users.unknown')),
    ...
    [user, state.sortBy, state.sortOrder, state.filterStatus, state.filterBranch, state.searchQuery, state.currentPage, state.viewMode, isRtl, language, t]
  );
  ...
  const updateOrderStatus = useCallback(
    async (orderId: string, newStatus: OrderStatus) => {
      ...
      try {
        ...
        toast.success(t('orders.status_updated_to', { status: t(`order_status.${newStatus}`) }), {
          position: isRtl ? 'top-left' : 'top-right',
          autoClose: 3000,
        });
      } catch (err: any) {
        console.error(`[${new Date().toISOString()}] Update order status error:`, err.message);
        toast.error(t('errors.status_update_failed', { message: err.message }), {
          position: isRtl ? 'top-left' : 'top-right',
          autoClose: 3000,
        });
      } finally {
  ...
  const updateItemStatus = useCallback(
    async (orderId: string, itemId: string, status: Order['items'][0]['status']) => {
      ...
      try {
        ...
        toast.success(t('orders.item_status_updated_to', { status: t(`item_status.${status}`) }), {
          position: isRtl ? 'top-left' : 'top-right',
          autoClose: 3000,
        });
      } catch (err: any) {
        console.error(`[${new Date().toISOString()}] Update item status error:`, err.message);
        toast.error(t('errors.item_status_update_failed', { message: err.message }), {
          position: isRtl ? 'top-left' : 'top-right',
          autoClose: 3000,
        });
      } finally {
      ...
      try {
        ...
        const items = state.assignFormData.items.map(item => ({
          ...
          assignedTo: state.chefs.find(chef => chef.userId === item.assignedTo) || { 
            _id: item.assignedTo, 
            name: t('users.unknown'), 
            displayName: t('users.unknown'),
            department: { _id: 'unknown', name: t('departments.unknown'), displayName: t('departments.unknown') } 
          },
          status: 'assigned',
        }));
        dispatch({ type: 'TASK_ASSIGNED', orderId, items });
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, ease: 'easeOut' }} className="mb-6">
          ...
          <Card className="p-3 mt-6 bg-white shadow-md border border-gray-200">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">{t('orders.search_label')}</label>
              ...
              <div>
                ...
                <Select
                  options={statusOptions.map(opt => ({
                    value: opt.value,
                    label: t(`order_status.${opt.label}`),
                  }))}
              ...
              <div>
                ...
                <Select
                  options={[{ value: '', label: t('orders.all_branches') }, ...state.branches.map(b => ({ value: b._id, label: b.displayName }))]}
