...
const DailySalesTable: React.FC<DailySalesTableProps> = ({ data, title, isRtl, loading, allBranches, daysInMonth, monthName }) => {
    ...
    return data
      ...
      .map(row => {
        ...
        if (selectedBranch !== 'all') {
          displayedDailySales = displayedDailySales.map((_, i) => displayedDailyBranchDetails[i][selectedBranch] || 0);
        }
        const displayedTotalSales = displayedDailySales.reduce((sum, q) => sum + q, 0);
        const displayedTotalValue = displayedTotalSales * (row.totalValue / row.totalSales || 0);
        return {
          ...row,
          displayedDailySales,
      ...
      }).sort((a, b) => b.displayedTotalSales - a.displayedTotalSales);
  ...
  const exportTable = (format: 'excel' | 'pdf') => {
    ...
    const rows = [
      ...filteredData.map((row, index) => ({
        ...
        product: row.product,
        unit: row.unit,
        ...Object.fromEntries(row.displayedDailySales.map((qty, i) => [displayedDays[i], qty])),
      ...
      })),
      {
        ...
        unit: '',
        ...Object.fromEntries(displayedDays.map((_, i) => [displayedDays[i], filteredData.reduce((sum, row) => sum + row.displayedDailySales[i], 0)])),
    ...
    if (format === 'excel') {
      ...
      ws['!cols'] = [{ wch: 10 }, { wch: 15 }, { wch: 20 }, { wch: 15 }, ...displayedDays.map(() => ({ wch: 12 })), { wch: 15 }, { wch: 15 }];
          ...
          <tbody className="divide-y divide-gray-200">
            {filteredData.map((row, index) => (
              <tr key={row.id} className={`hover:bg-blue-50 transition-colors duration-200 ${isRtl ? 'flex-row-reverse' : ''}`}>
                ...
                {row.displayedDailySales.map((qty, i) => (
                  <td
                    key={i}
                    ...
                    data-tooltip-content={getTooltipContent(qty, selectedBranch === 'all' ? row.displayedDailyBranchDetails[i] : {}, isRtl, 'sales')}
                  >
