...
interface DailyReturnsTableProps {
  data: ReturnRow[];
  title: string;
  month: number;
  isRtl: boolean;
...
const DailyReturnsTable: React.FC<DailyReturnsTableProps> = ({ data, title, isRtl, loading, allBranches, daysInMonth, monthName }) => {
    ...
    return data
      ...
      .map(row => {
        ...
        if (selectedBranch !== 'all') {
          displayedDailyReturns = displayedDailyReturns.map((_, i) => displayedDailyBranchDetails[i][selectedBranch] || 0);
        }
        const displayedTotalReturns = displayedDailyReturns.reduce((sum, q) => sum + q, 0);
        const displayedTotalValue = displayedTotalReturns * (row.totalValue / row.totalReturns || 0);
        return {
          ...row,
      ...
      }).sort((a, b) => b.displayedTotalReturns - a.displayedTotalReturns);
  ...
  const grandTotalValue = filteredData.reduce((sum, row) => sum + row.displayedTotalValue, 0);
  ...
  const getTooltipContent = (dailyQuantity: number, branchDetails: { [branch: string]: number }, isRtl: boolean, type: string) => {
    let header = isRtl ? 'مرتجعات' : 'Returns';
  ...
  const exportTable = (format: 'excel' | 'pdf') => {
    ...
    const rows = [
      ...filteredData.map((row, index) => ({
        ...
        product: row.product,
        unit: row.unit,
        ...Object.fromEntries(row.displayedDailyReturns.map((qty, i) => [displayedDays[i], qty])),
        totalReturns: row.displayedTotalReturns,
        totalValue: formatPrice(row.displayedTotalValue, isRtl),
        ratio: row.totalOrders > 0 ? ((row.displayedTotalReturns / row.totalOrders) * 100).toFixed(2) : '0.00',
      })),
      {
        no: '',
        code: '',
        ...
        unit: '',
        ...Object.fromEntries(displayedDays.map((_, i) => [displayedDays[i], filteredData.reduce((sum, row) => sum + row.displayedDailyReturns[i], 0)])),
    ...
    if (format === 'excel') {
      ...
      ws['!cols'] = [{ wch: 10 }, { wch: 15 }, { wch: 20 }, { wch: 15 }, ...displayedDays.map(() => ({ wch: 12 })), { wch: 15 }, { wch: 15 }, { wch: 15 }];
      ...
      >
        <table className="min-w-full divide-y divide-gray-200 text-xs">
          ...
          <tbody className="divide-y divide-gray-200">
            {filteredData.map((row, index) => {
              const ratio = row.totalOrders > 0 ? (row.displayedTotalReturns / row.totalOrders) * 100 : 0;
              return (
                <tr key={row.id} className={`hover:bg-blue-50 transition-colors duration-200 ${isRtl ? 'flex-row-reverse' : ''}`}>
                  ...
                  {row.displayedDailyReturns.map((qty, i) => (
                    <td
                      key={i}
                      className={`px-4 py-3 text-center font-medium ${qty > 0 ? 'bg-red-50 text-red-700' : 'text-gray-700'}`}
                      data-tooltip-id="return-tooltip"
                      data-tooltip-content={getTooltipContent(qty, selectedBranch === 'all' ? row.displayedDailyBranchDetails[i] : {}, isRtl, 'return')}
                    >
                  ...
                  <td className="px-4 py-3 text-gray-700 text-center font-medium">{formatPrice(row.displayedTotalValue, isRtl)}</td>
                  <td className={`px-4 py-3 text-center font-medium ${getRatioColor(ratio)}`}>
                    {formatNumber(ratio.toFixed(2), isRtl) + '%'}
                  </td>
                </tr>
              );
            })}
        ...
        <Tooltip id="return-tooltip" place="top" className="custom-tooltip whitespace-pre-line" />
      </motion.div>
    </div>
  );
};

export default DailyReturnsTable;