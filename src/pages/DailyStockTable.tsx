...
const DailyStockTable: React.FC<DailyStockTableProps> = ({ data, title, type, isRtl, loading, allBranches, daysInMonth, monthName }) => {
  ...
  const filteredData = useMemo(() => {
    return data
      ...
      .map(row => {
        ...
        let displayedDailyReturnsDetails = (row.dailyReturnsDetails || []).slice(startDay - 1, endDay);
        if (selectedBranch !== 'all') {
          displayedDailyQuantities = displayedDailyQuantities.map((_, i) => displayedDailyBranchDetails[i][selectedBranch] || 0);
          displayedDailySales = displayedDailySales.map((_, i) => displayedDailySalesDetails[i][selectedBranch] || 0);
          displayedDailyReturns = displayedDailyReturns.map((_, i) => displayedDailyReturnsDetails[i][selectedBranch] || 0);
        }
        ...
        return {
          ...
          displayedDailySales,
          displayedDailySalesDetails,
          displayedDailyReturns,
          displayedDailyReturnsDetails,
  ...
  const getTooltipContent = (dailyQuantity: number, branchDetails: { [branch: string]: number }, isRtl: boolean, type: string) => {
    let header = type === 'in' ? (isRtl ? 'زيادة مخزون' : 'Stock In') : (isRtl ? 'نقص مخزون' : 'Stock Out');
  ...
  return (
    <div className="mb-8">
      ...
      >
        ...
        <Tooltip id="stock-tooltip" place="top" className="custom-tooltip whitespace-pre-line" />
      </motion.div>
    </div>
  );
};
