...
const ProductionReport = () => {
  ...
  const currentDate = new Date('2025-10-12T09:25:00+03:00'); // 09:25 AM EEST, October 12, 2025
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const response = await api.get('/inventory', { params: { branch: user?.branchId } });
        const inventoryData = response.data.inventory;

        // توزيع الطلبات (فارغ حاليًا بسبب غياب orders)
        const orderDistribution = new Map();
        // سيظل فارغًا لأن orders هي []
