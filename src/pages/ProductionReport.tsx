...
const ProductionReport: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'orders' | 'stockIn' | 'stockOut' | 'returns' | 'sales' | 'ordersVsSales' | 'ordersVsReturns'>('orders');
    ...
    <div className={`min-h-screen px-6 py-8 ${isRtl ? 'rtl font-amiri' : 'ltr font-inter'} bg-gray-100`}>
      <div className="mb-8 bg-white shadow-md rounded-xl p-4">
        ...
        <div className={`flex flex-wrap gap-2 justify-center ${isRtl ? 'flex-row-reverse' : ''}`}>
          ...
          >
            {isRtl ? 'حركة الطلبات مقابل المرتجعات' : 'Orders vs Returns'}
          </Button>
        </div>
      </div>
      <AnimatePresence>
        ...
        {activeTab === 'ordersVsReturns' && (
          <OrdersVsReturnsTable
            ...
            title={isRtl ? 'تقرير حركة الطلبات مقابل المرتجعات' : 'Orders vs Returns Report'}
            month={selectedMonth}
            isRtl={isRtl}
            loading={loading}
            allBranches={allBranches}
