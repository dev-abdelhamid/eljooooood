            ...
            <tr className={`font-semibold bg-gray-50 ${isRtl ? 'flex-row-reverse' : ''}`}>
              ...
              {daysInMonth.map((_, i) => (
                <Fragment key={i}>
                  ...
                  <td className="px-4 py-3 text-gray-800 text-center">
                    {(() => {
                      const dailyReturnsTotal = filteredData.reduce((sum, row) => sum + (row.dailyReturns[i] || 0), 0);
                      const dailyOrdersTotal = filteredData.reduce((sum, row) => sum + (row.dailyOrders[i] || 0), 0);
...
const ProductionReport: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // Process orders - removed incorrect fallback
        ...
        for (let month = 0; month < 12; month++) {
          ...
          const ordersDailyMap = new Map<string, { dailyOrders: number[]; dailyBranchDetails: { [branch: string]: number }[]; totalOrders: number; totalPrice: number; code: string; product: string; unit: string }>();
          ...
          orders.forEach((order: any) => {
            ...
            if (year === currentYear && orderMonth === month) {
              ...
              (order.items || []).forEach((item: any) => {
                ...
                // Daily orders for new table with branch details
              ...
              if (year === currentYear && prodMonth === month) {
                ...
                if (movement.type === 'in') {
                  ...
                  if (isReturn) {
                    ...
                    returnRow.totalValue += Math.abs(quantity) * details.price;
                    ...
                    salesRow.totalValue += Math.abs(quantity) * details.price;
                  }
                }
              }
            });
          ...
          // Process orders vs returns with all products
          const ordersVsReturnsMap = new Map<string, OrdersVsReturnsRow>();
          for (const productId of productDetails.keys()) {
            const key = `${productId}-${month}`;
            const details = productDetails.get(productId)!;
            ...
            ordersVsReturnsMap.set(key, {
              ...
              dailyOrdersDetails: ordersRow.dailyBranchDetails,
              dailyReturnsDetails: returnsRow.dailyBranchDetails,
  ...
  const handleExportExcelOrdersVsReturns = () => {
    ...
    const allHeaders = [...headers, ...dayHeaders, ...totalHeaders];
    ...
    const rows = [
      ...
      })),
      {
        ...
        totalOrders: data.reduce((sum, row) => sum + row.totalOrders, 0),
        totalReturns: data.reduce((sum, row) => sum + row.totalReturns, 0),
        totalRatio: (data.reduce((sum, row) => sum + row.totalOrders, 0) > 0) ? ((data.reduce((sum, row) => sum + row.totalReturns, 0) / data.reduce((sum, row) => sum + row.totalOrders, 0)) * 100).toFixed(2) : '0.00',
      },
    ];

    const ws = XLSX.utils.json_to_sheet(isRtl ? rows.map(row => Object.fromEntries(Object.entries(row).reverse())) : rows, { header: allHeaders });
    if (isRtl) ws['!views'] = [{ RTL: true }];
  ...
  const handleExportPDFOrdersVsReturns = () => {
    const data = ordersVsReturnsData[selectedMonth] || [];
    const monthName = months[selectedMonth].label;
    const headers = [
      isRtl ? 'رقم' : 'No.',
    ...
    const dataRows = data.map(row => [
      '', // Placeholder for no, but since not used in PDF, or add index
      row.code,
      row.product,
      row.unit,
      ...daysInMonth.flatMap((_, i) => [
        row.dailyOrders[i],
        row.dailyReturns[i],
        row.dailyOrders[i] > 0 ? ((row.dailyReturns[i] / row.dailyOrders[i]) * 100).toFixed(2) : '0.00',
      ]),
      row.totalOrders,
      row.totalReturns,
      row.totalRatio.toFixed(2),
    ]);

    exportToPDF(dataRows, 'Orders vs Returns', monthName, allHeaders, isRtl, data.length, data.reduce((sum, row) => sum + row.totalOrders, 0), data.reduce((sum, row) => sum + row.totalReturns, 0), []);
  };
      ...
      <AnimatePresence>
        ...
        {activeTab === 'ordersVsReturns' && (
          <DailyOrdersVsReturnsComponent
            ...
            onExportExcel={handleExportExcelOrdersVsReturns}
            onExportPDF={handleExportPDFOrdersVsReturns}
          />
        )}
      </AnimatePresence>
    </div>
