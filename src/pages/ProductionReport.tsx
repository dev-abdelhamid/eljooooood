  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // Process orders without fallback
        ...
        for (let month = 0; month < 12; month++) {
          ...
          // Initialize maps with all products
          productDetails.forEach((details, productId) => {
            ...
            orderMap.set(key, {
              ...
              dailyBranchDetails: Array.from({ length: daysInMonthCount }, () => Object.fromEntries(fetchedBranches.map(b => [b.displayName, 0]))),
              totalQuantity: 0,
              totalPrice: 0,
              sales: 0,
              actualSales: 0,
            ...
            stockOutMap.set(key, {
              ...
              dailySalesDetails: Array.from({ length: daysInMonthCount }, () => Object.fromEntries(fetchedBranches.map(b => [b.displayName, 0]))),
              dailyReturnsDetails: Array.from({ length: daysInMonthCount }, () => Object.fromEntries(fetchedBranches.map(b => [b.displayName, 0]))),
            });
            ...
            ordersVsReturnsMap.set(key, {
              ...
              dailyReturns: Array(daysInMonthCount).fill(0),
              dailyOrdersBranchDetails: Array.from({ length: daysInMonthCount }, () => Object.fromEntries(fetchedBranches.map(b => [b.displayName, 0]))),
              dailyReturnsBranchDetails: Array.from({ length: daysInMonthCount }, () => Object.fromEntries(fetchedBranches.map(b => [b.displayName, 0]))),
              totalOrders: 0,
              totalReturns: 0,
              totalRatio: 0,
            });
          ...
          orders.forEach((order: any) => {
            ...
            if (year === currentYear && orderMonth === month) {
              ...
              (order.items || []).forEach((item: any) => {
                ...
                const ovrRow = ordersVsReturnsMap.get(key)!;
                ovrRow.dailyOrders[day] += quantity;
                ovrRow.dailyOrdersBranchDetails[day][branch] = (ovrRow.dailyOrdersBranchDetails[day][branch] || 0) + quantity;
                ovrRow.totalOrders += quantity;
                ovrRow.totalRatio = ovrRow.totalOrders > 0 ? (ovrRow.totalReturns / ovrRow.totalOrders) * 100 : 0;
              });
            }
          });
              ...
              if (year === currentYear && prodMonth === month) {
                ...
                if (movement.type === 'in') {
                  ...
                  if (isReturn) {
                    ...
                    const ovrRow = ordersVsReturnsMap.get(key)!;
                    ovrRow.dailyReturns[day] += qty;
                    ovrRow.dailyReturnsBranchDetails[day][branchName] = (ovrRow.dailyReturnsBranchDetails[day][branchName] || 0) + qty;
                    ovrRow.totalReturns += qty;
                    ovrRow.totalRatio = ovrRow.totalOrders > 0 ? (ovrRow.totalReturns / ovrRow.totalOrders) * 100 : 0;
                  } else if (isSale) {
