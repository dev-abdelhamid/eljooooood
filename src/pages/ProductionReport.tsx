    ...
    const fetchData = async () => {
      ...
      try {
        ...
        for (let month = 0; month < 12; month++) {
          ...
          // Process returns from returnsAPI
          const returnsResponse = await returnsAPI.getAll({ month });
          returnsResponse.forEach((ret: any) => {
            ...
            if (year === currentYear && returnMonth === month) {
              ...
              ret.items.forEach((item: any) => {
                const product = item.product.displayName || (isRtl ? 'منتج غير معروف' : 'Unknown Product');
                const key = `${product}-${month}`;
                ...
                row.totalValue += quantity * item.product.price;
              });
            }
          });

          // Process sales from salesAPI
          const salesItems = salesResponse.productSales;
          salesItems.forEach((salesItem: any) => {
            const product = salesItem.displayName;
            const key = `${product}-${selectedMonth}`;
            if (!salesMap.has(key)) {
              salesMap.set(key, {
                id: key,
                code: salesItem.productId,
                product,
                unit: salesItem.unit || 'unit',
                branchQuantities: {},
                totalQuantity: 0,
                totalPrice: 0,
                sales: salesItem.totalRevenue,
                actualSales: salesItem.totalQuantity,
              });
            }
            const row = salesMap.get(key)!;
            row.totalQuantity += salesItem.totalQuantity;
            row.totalPrice += salesItem.totalRevenue;
          });
  ...
  return (
    <div className={`min-h-screen px-6 py-8 ${isRtl ? 'rtl font-amiri' : 'ltr font-inter'} bg-gray-100`}>
      ...
      <AnimatePresence mode="wait">
        ...
        {activeTab === 'returns' && (
          <motion.div key="returns" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.3 }}>
            {renderReturnTable(returnData[selectedMonth] || [], isRtl ? 'تقرير حركة المرتجعات' : 'Return Movements Report', selectedMonth)}
          </motion.div>
        )}
        {activeTab === 'sales' && (
          <motion.div key="sales" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.3 }}>
            {renderSalesTable(salesData[selectedMonth] || [], isRtl ? 'تقرير حركة المبيعات' : 'Sales Movements Report', selectedMonth)}
          </motion.div>
        )}
      </AnimatePresence>
    </div>
