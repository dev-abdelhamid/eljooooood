...
const translations = {
  ar: {
    ...
    departmentRequired: 'الأقسام مطلوبة',
  ...
  en: {
    ...
    departmentRequired: 'Departments are required',
  ...
  return (
    <div className="relative group">
      ...
      <input
        ...
        className={`w-full ${isRtl ? 'pl-10 pr-2' : 'pr-10 pl-2'} py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-amber-500 focus focus:border-transparent transition-all duration-300 bg-white shadow-sm hover:shadow-md text-xs placeholder-gray-400 ${isRtl ? 'text-right' : 'text-left'}`}
...
export function Chefs() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      setChefs(fetchedChefs.map((chef: any) => ({
        ...
        department: chef.department.map((dept: any) => ({
          id: dept._id,
          name: dept.name,
          nameEn: dept.nameEn,
          code: dept.code,
  ...
  const validateForm = () => {
    ...
    if (selectedDepartments.length === 0) errors.department = t.departmentRequired;
  ...
  const openEditModal = (chef: Chef) => {
    ...
    setSelectedDepartments(chef.department.map(d => d.id));
    setIsEditMode(true);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isEditMode && selectedChef) {
        await chefsAPI.update(selectedChef.id, chefData);
        toast.success(t.updated, { position: isRtl ? 'top-right' : 'top-left' });
      } else {
        await chefsAPI.create(chefData);
      ...
      if (err.message) {
        errorMessage =
          ...
          err.message.includes('قسم') ? err.message : errorMessage;
      }
          ...
          >
            {filteredChefs.map((chef) => (
              <motion.div
                ...
                className="p-4 bg-white rounded-xl shadow-sm hover:shadow-md transition-all duration-200 flex flex-col justify-between cursor-pointer"
