          ...
          >
            {options.map((option) => (
              <button
                ...
                className="w-full px- LH 4 py-2.5 text-sm text-gray-700 hover:bg-amber-50 hover:text-amber-600 transition-colors duration-200 text-left"
              >
                {option.label}
              </button>
            ))}
        ...
        <div className="flex flex-col gap-4">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <h2 className="text-lg font-bold text-gray-800">
              {isRtl ? 'ملخص الطلبات' : 'Orders Summary'} - {dateRange.label}
            </h2>
