...
const exportToPDF = async (
  ...
  try {
    ...
    const fileName = generateFileName('DailyOrdersByBranch', periodLabel, isRtl, 'pdf');
    doc.save(fileName);
...
const exportToExcel = (dataRows: any[], headers: string[], periodLabel: string, isRtl: boolean) => {
  ...
  try {
    ...
    XLSX.writeFile(wb, generateFileName('DailyOrdersByBranch', periodLabel, isRtl, 'xlsx'));
...
const DailyOrdersSummary: React.FC = () => {
  ...
  const { start, end, label } = useMemo(() => {
    ...
    if (period === 'daily') {
      ...
      e = endOfDay(endDate);
      l = `${format(startDate, 'dd/MM/yyyy')} - ${format(endDate, 'dd/MM/yyyy')}`;
    }
  ...
  const exportTable = (format: 'excel' | 'pdf') => {
    const allBranches = branches.map(b => b.displayName).sort((a, b) => a.localeCompare(b, language));
    const headers = [
      isRtl ? 'الاسم' : 'Name',
      isRtl ? 'السعر' : 'Price',
      isRtl ? 'الكود' : 'Code',
    ...
    if (format === 'pdf') {
      exportToPDF(rows, isRtl ? 'ملخص الطلبات حسب الفرع' : 'Orders Summary by Branch', label, headers, isRtl, filteredData.length, grandTotalQuantity, grandTotalPrice);
    } else {
        ...
        <div className="flex flex-col gap-4">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <h2 className="text-lg font-bold text-gray-800">
              {isRtl ? 'ملخص الطلبات حسب الفرع' : 'Orders Summary by Branch'} - {label}
            </h2>
          ...
          <table className="min-w-full divide-y divide-gray-200 text-xs">
            <thead className="bg-amber-50 sticky top-0 z-10">
              <tr className={isRtl ? 'flex-row-reverse' : ''}>
                ...
                {branches.map(branch => (
                  <th key={branch._id} className="px-2 py-3 font-semibold text-gray-700 text-center min-w-[80px]">
                    {branch.displayName}
                  </th>
                ))}
              ...
              {filteredData.map(row => (
                <tr key={row.id} className="hover:bg-amber-50 transition-colors duration-200">
                  ...
                  {branches.map(branch => (
                    ...
                    >
                      {row.branchQuantities[branch.displayName] !== 0 ? formatNumber(row.branchQuantities[branch.displayName] || 0, isRtl) : '0'}
                    </td>
                  ))}
