...
const Button: React.FC<{
  ...
  <button
    onClick={onClick}
    disabled={disabled}
    className={`flex items-center gap-2 rounded-lg px-5 py-2.5 text-sm font-bold transition-all duration-200 shadow-sm ${
  ...
  return (
    <div className={`relative group w-full ${className}`}>
      ...
      <input
        ...
        className={`w-full ${isRtl ? 'pl-12 pr-4' : 'pr-12 pl-4'} px-4 py-2.5 border border-gray-300 rounded-xl focus:ring-2 focus:ring-amber-500 focus:border-transparent transition-all bg-white shadow-sm hover:shadow-md text-base placeholder-gray-500 ${isRtl ? 'text-right font-amiri' : 'text-left font-inter'}`}
  ...
  return (
    <div className={`relative group ${className}`} ref={dropdownRef}>
      <button
        ...
        className={`w-full px-4 py-2.5 border border-gray-200 rounded-xl focus:ring-2 focus:ring-amber-500 focus:border-transparent transition-all bg-white shadow-sm hover:shadow-md text-sm font-medium text-gray-700 ${isRtl ? 'text-right font-amiri' : 'text-left font-inter'} flex justify-between items-center ${disabled ? 'opacity-50 cursor-not-allowed' : ''}`}
        aria-label={ariaLabel}
      >
      ...
      <AnimatePresence>
        {isOpen && !disabled && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -10 }}
            transition={{ duration: 0.2 }}
            className="absolute z-50 w-full mt-2 bg-white border border-gray-100 rounded-xl shadow-xl max-h-60 overflow-y-auto"
          >
            {options.map((option) => (
              <button
                key={option.value}
                ...
                className="w-full px-4 py-2.5 text-sm text-gray-700 hover:bg-amber-50 hover:text-amber-600 transition-colors text-left font-medium"
              >
                {option.label}
              </button>
            ))}
...
const generatePDFHeader = (
  ...
  doc.text(title, titleX, 30, { align: isRtl ? 'right' : 'left' });

  // الفترة
  doc.setFontSize(14);
  doc.setTextColor(255, 255, 255);
  const periodX = isRtl ? margin + 10 : pageWidth - margin - 10;
  doc.text(`الفترة: ${periodLabel}`, periodX, 42, { align: isRtl ? 'left' : 'right' });
  ...
  stats.forEach((stat, i) => {
    ...
    doc.text(stat, x, 65 + i * 8, { align: isRtl ? 'left' : 'right' });
  });
...
const generatePDFTable = (
  ...
  autoTable(doc, {
    ...
    theme: 'grid',
    startY: 80,
    margin: { top: 80, bottom: 20, left: margin, right: margin },
    tableWidth: 'auto',
    ...
    didParseCell: (hookData) => {
      if (hookData.section === 'body' || hookData.section === 'head') {
        hookData.cell.text = hookData.cell.text.map(text => {
          ...
          // تحويل الأرقام فقط في الأعمدة التي تحتاج
...
const DailyOrdersSummary: React.FC = () => {
  ...
  return (
    <div className={`min-h-screen px-4 py-8 ${isRtl ? 'rtl font-amiri' : 'ltr font-inter'} bg-gradient-to-br from-gray-50 to-gray-100`}>
      <div className="mb-6 bg-white shadow-xl rounded-2xl p-6 border border-gray-200">
            ...
            {selectedPeriod === 'custom' && (
              <div className="flex gap-2 items-center w-full sm:w-auto">
                <input
                  ...
                  className="px-4 py-2.5 border border-gray-300 rounded-xl text-sm w-full sm:w-auto focus:ring-2 focus:ring-amber-500"
                />
      ...
      {loading ? (
        <div className="flex justify-center items-center py-16">
          <div className="animate-spin rounded-full h-14 w-14 border-t-4 border-b-4 border-amber-600"></div>
        </div>
      ) : filteredData.length === 0 ? (
        <div className="text-center py-16 text-gray-600	bg-white rounded-2xl shadow-md p-8">
          <p className="text-lg font-medium">{isRtl ? 'لا توجد بيانات في هذه الفترة' : 'No data for this period'}</p>
        </div>
      ) : (
        <motion.div
        ...
        >
          <table className="min-w-full divide-y divide-gray-200 text-sm">
            <thead className="bg-gradient-to-r from-amber-50 to-amber-100 sticky top-0 z-10">
              <tr className={isRtl ? 'flex-row-reverse' : ''}>
                <th className="px-4 py-4 font-bold text-gray-700 text-center min-w-[160px]">{isRtl ? 'الاسم' : 'Name'}</th>
                <th className="px-3 py-4 font-bold text-gray-700 text-center min-w-[80px]">{isRtl ? 'السعر' : 'Price'}</th>
                <th className="px-3 py-4 font-bold text-gray-700 text-center min-w-[80px]">{isRtl ? 'الكود' : 'Code'}</th>
                {allBranches.map((branch) => (
                  <th key={branch} className="px-3 py-4 font-bold text-gray-700 text-center min-w-[90px] break-words">
                    {branch}
                  </th>
                ))}
                <th className="px-4 py-4 font-bold text-gray-700 text-center min-w-[90px]">{isRtl ? 'الإجمالي' : 'Total'}</th>
                <th className="px-3 py-4 font-bold text-gray-700 text-center min-w-[80px]">{isRtl ? 'وحدة' : 'Unit'}</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-200">
              {filteredData.map((row) => (
                <tr key={row.id} className="hover:bg-amber-50 transition-colors duration-200">
                  <td className="px-4 py-3 text-gray-700 text-center truncate max-w-[160px]" title={row.product}>
                    {row.product}
                  </td>
                  <td className="px-3 py-3 text-gray-700 text-center font-medium">{formatPrice(row.price, isRtl)}</td>
                  <td className="px-3 py-3 text-gray-700 text-center">{row.code}</td>
                  {allBranches.map((branch) => (
                    <td
                      key={branch}
                  ...
                  <td className="px-4 py-3 text-gray-700 text-center font-bold">{formatNumber(row.totalQuantity, isRtl)}</td>
                  <td className="px-3 py-3 text-gray-700 text-center">{row.unit}</td>
                </tr>
              ))}
              <tr className={`font-bold bg-gradient-to-r from-gray-100 to-gray-200 ${isRtl ? 'flex-row-reverse' : ''}`}>
                <td className="px-4 py-4 text-gray-800 text-center">{isRtl ? 'الإجمالي' : 'Total'}</td>
                <td className="px-3 py-4 text-gray-800 text-center">{formatPrice(grandTotalPrice, isRtl)}</td>
                <td className="px-3 py-4 text-gray-800 text-center"></td>
                {allBranches.map((branch) => (
                  <td key={branch} className="px-3 py-4 text-gray-800 text-center">
                    {formatNumber(filteredData.reduce((sum, row) => sum + (row.branchQuantities[branch] || 0), 0), isRtl)}
                  </td>
                ))}
                <td className="px-4 py-4 text-gray-800 text-center">{formatNumber(grandTotalQuantity, isRtl)}</td>
                <td className="px-3 py-4 text-gray-800 text-center"></td>
              </tr>
            </tbody>
          </table>
          <Tooltip
            id="branch-tooltip"
            place="top"
            className="z-[9999] bg-white border border-gray-300 rounded-lg p-3 shadow-2xl max-w-xs text-xs text-gray-800 font-medium"
          />
        </motion.div>
      )}
    </div>
