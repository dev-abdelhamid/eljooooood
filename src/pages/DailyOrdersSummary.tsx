...
const formatPrice = (amount: number, isRtl: boolean): string => {
  ...
  return isRtl ? `${toArabicNumerals(validAmount.toFixed(2))} ر.س` : `${validAmount.toFixed(2)} SAR`;
};
...
const generatePDFTable = (
  ...
  const columnWidths = [20, 15, 15, ...Array(numColumns - 5).fill(15), 20, 15].map(w => Math.min(w, maxTableWidth / numColumns));
...
const DailyOrdersSummary: React.FC = () => {
  ...
  const isRtl = language === 'ar';
  const { user } = useAuth();
  const today = new Date('2025-10-21T11:40:00');
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [inventory, ordersResponse, branchesResponse, salesResponse] = await Promise.all([
        ...
        branchesAPI.getAll(),
        salesAPI.getAnalytics({ lang: language }),
      ]);
      ...
      if (orders.length === 0) {
        ...
        orders = inventory
          ...
          .flatMap((item: any) => (item.movements || []).map((movement: any) => ({
            status: 'completed',
            createdAt: movement.createdAt || today.toISOString(),
      ...
      const getDateRange = () => {
        ...
        if (selectedPeriod === 'daily') start.setHours(0, 0, 0, 0);
