      ...
      <div className={`mb-10 ${isRtl ? 'flex-row-reverse' : ''}`}>
        <div className="flex flex-col lg:flex-row justify-between items-center gap-6">
          <div className="flex items-center gap-4">
            <h2 className="text-2xl font-bold text-gray-900">{isRtl ? 'ملخص الطلبات اليومية' : 'Daily Orders Summary'}</h2>
            <span className="text-lg font-medium text-gray-600"> - {monthName}</span>
            <span className="text-lg font-medium text-gray-600">| إجمالي المنتجات: {filteredData.length}</span>
            <span className="text-lg font-medium text-gray-600">| إجمالي الكمية: {formatNumber(grandTotalQuantity, isRtl)}</span>
            <span className="text-lg font-medium text-gray-600">| إجمالي المبلغ: {formatPrice(grandTotalPrice, isRtl)}</span>
          </div>
