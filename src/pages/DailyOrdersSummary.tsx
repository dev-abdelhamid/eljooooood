...
const generatePDFHeader = (
  ...
  const currentDate = new Date('2025-10-21T17:37:00').toLocaleDateString(isRtl ? 'ar-SA' : 'en-US', {
...
const DailyOrdersSummary: React.FC = () => {
  ...
  const getDateRange = useCallback(() => {
    const now = new Date('2025-10-21T17:37:00'); // Updated to 05:37 PM EEST
    ...
    try {
      ...
      orders.forEach((order: any) => {
        ...
        (order.items || []).forEach((item: any) => {
          ...
          let details = productDetails.get(productId);
          if (!details) {
            console.warn(`Product ID ${productId} not found in productDetails, using fallback`);
            details = {
              code: 'N/A',
