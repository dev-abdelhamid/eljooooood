...
import { ProductDropdown  , ProductSearchInput} from './NewOrder';
...
const InventoryDashboard: React.FC<{
  ...
  isRtl: boolean;
  language: string;
}> = React.memo(({ inventory, transactions, isRtl, language }) => {
  const [filter, setFilter] = useState<FilterState>({ status: 'all', search: '', timeFilter: 'week' });
  ...
  const filteredTransactions = useMemo(() => {
    return transactions
      .filter((trans) => filter.status === 'all' || trans.type === filter.status)
      .filter(
        (trans) =>
          (isRtl ? trans.productName : trans.productNameEn || trans.productName).toLowerCase().includes(filter.search.toLowerCase())
      )
      .sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
  }, [transactions, filter.status, filter.search, isRtl]);
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between mb-3 gap-3">
        ...
        <div className="flex items-center gap-2 w-full sm:w-auto">
          ...
          <ProductSearchInput
            ...
            placeholder={isRtl ? 'ابحث عن المنتج' : 'Search by product'}
            className="w-full sm:w-40"
          />
          <select
            value={filter.status}
            ...
            aria-label={isRtl ? 'نوع المعاملة' : 'Transaction Type'}
          >
            <option value="all">{isRtl ? 'الكل' : 'All'}</option>
            <option value="add">{isRtl ? 'إضافة' : 'Add'}</option>
            <option value="remove">{isRtl ? 'إزالة' : 'Remove'}</option>
          </select>
        </div>
      </div>
      <div className="space-y-4">
        <div>
          <h4 className="text-sm font-semibold text-gray-700 mb-2">{isRtl ? 'المنتجات التي تحتاج تجديد' : 'Low Stock Items'}</h4>
          {lowStockItems.length === 0 ? (
            <p className="text-gray-500 text-xs">{isRtl ? 'لا توجد منتجات تحتاج تجديد' : 'No items need restocking'}</p>
          ) : (
              ...
              <AnimatePresence>
                {lowStockItems.map((item) => (
                  <motion.div
                    ...
                    className="border border-red-100 rounded-lg p-2 bg-red-50 shadow-sm"
                  >
                    <p className="text-xs font-medium text-gray-800">
                      {isRtl ? item.productName : item.productNameEn || item.productName}
                    </p>
                    <p className="text-xs text-gray-600">
                      {isRtl
                        ? `الكمية: ${item.quantity} ${getDisplayUnit(item.unit, item.unitEn)} (الحد الأدنى: ${item.minQuantity})`
                        : `Quantity: ${item.quantity} ${getDisplayUnit(item.unit, item.unitEn)} (Min: ${item.minQuantity})`}
                    </p>
                  </motion.div>
                ))}
              </AnimatePresence>
        ...
        <div>
          <h4 className="text-sm font-semibold text-gray-700 mb-2">{isRtl ? 'سجل المعاملات الحديثة' : 'Recent Transactions'}</h4>
          {filteredTransactions.length === 0 ? (
            <p className="text-gray-500 text-xs">{isRtl ? 'لا توجد معاملات حديثة' : 'No recent transactions'}</p>
          ) : (
            <div className="space-y-2 max-h-40 overflow-y-auto">
              <AnimatePresence>
                {filteredTransactions.map((trans) => (
                  ...
                  >
                    <p className="text-xs font-medium text-gray-800">
                      {isRtl ? trans.productName : trans.productNameEn || trans.productName}
                    </p>
                    <p className="text-xs text-gray-600">
                      {isRtl
                        ? `${trans.type === 'add' ? 'إضافة' : 'إزالة'}: ${trans.quantity} ${getDisplayUnit(trans.unit, trans.unitEn)}`
                        : `${trans.type === 'add' ? 'Added' : 'Removed'}: ${trans.quantity} ${getDisplayUnit(trans.unit, trans.unitEn)}`}
                    </p>
                    <p className="text-xs text-gray-500">{formatDate(trans.createdAt, language)}</p>
                  </motion.div>
                ))}
              </AnimatePresence>
            </div>
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white p-3 rounded-lg shadow-sm border border-gray-100">
        <div className="flex flex-col sm:flex-row items-center justify-between mb-3 gap-3">
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto">
            <ProductDropdown
              products={orders.flatMap((order) => order.items.map((item) => ({
              ...
              selectedProductId={filter.status}
              onSelect={(value) => setFilter((prev) => ({ ...prev, status: value || 'all' }))}
              placeholder={isRtl ? 'حالة المهمة' : 'Task Status'}
              className="w-full sm:w-40"
            />
            <ProductSearchInput
              value={filter.search}
...
export const Dashboard: React.FC = () => {
      ...
      try {
        ...
        if (user.role === 'chef') {
          ...
          const promises = [
            ...
            user.role === 'branch' ? inventoryAPI.getAll({ branchId: user.id || user._id }).catch(() => []) : Promise.resolve([]),
            user.role === 'branch' ? inventoryAPI.getRecentTransactions({ branchId: user.id || user._id, limit: 10 }).catch(() => []) : Promise.resolve([]),
          ];
          [ordersResponse, tasksResponse, chefsResponse, branchesResponse, inventoryResponse, transactionsResponse] = await Promise.all(promises);
        }
        ...
        const mappedTransactions = transactionsResponse.map((trans: any) => ({
          id: trans._id || crypto.randomUUID(),
          productId: trans.product?._id || 'unknown',
          productName: trans.product?.name || (isRtl ? 'منتج غير معروف' : 'Unknown Product'),
          productNameEn: trans.product?.nameEn || trans.product?.name || 'Unknown',
          quantity: Number(trans.quantity) || 0,
          unit: trans.product?.unit || 'غير محدد',
          unitEn: trans.product?.unitEn || trans.product?.unit || 'N/A',
          type: trans.type || 'add',
          createdAt: formatDate(trans.createdAt || new Date(), language),
        }));
        ...
        const totalInventoryValue = mappedInventory.reduce((sum, item) => sum + item.quantity * item.price, 0);
        const lowStockItems = mappedInventory.filter((item) => item.quantity <= item.minQuantity).length;
        ...
        const errorMessage = err.status === 403 ? (isRtl ? 'الوصول غير مصرح به' : 'Unauthorized access') : (isRtl ? 'خطأ في جلب البيانات' : 'Error fetching data');
        setError(errorMessage);
  ...
  useEffect(() => {
    ...
    socket.on('orderDelivered', (data: any) => {
      ...
      addNotification({
        ...
        data: { orderId: orders.find(o => o.orderNumber === data.orderNumber)?.id, eventId: data.eventId },
        read: false,
        createdAt: formatDate(new Date(), language),
    ...
    socket.on('inventoryUpdated', (data: any) => {
      if (!data.productId || !data.productName || !data.quantity || !data.eventId || !data.branchId) return;
      if (user.role !== 'branch' || data.branchId !== user.branchId) return;

      setInventory((prev) =>
        prev.map((item) =>
          item.productId === data.productId ? { ...item, quantity: Number(data.quantity) } : item
        )
      );
      ...
      addNotification({
        ...
        type: 'info',
        message: isRtl
          ? `تم تحديث المخزون: ${data.productName} (${data.quantity} ${isRtl ? data.unit : data.unitEn || data.unit})`
          : `Inventory updated: ${data.productName} (${data.quantity} ${isRtl ? data.unit : data.unitEn || data.unit})`,
        data: { productId: data.productId, eventId: data.eventId },
        read: false,
        createdAt: formatDate(new Date(), language),
        sound: '/sounds/inventory-updated.mp3',
        vibrate: [300, 100, 300],
        path: '/dashboard',
      });
      fetchDashboardData(true);
    ...
    >
      ...
      {user.role === 'branch' && (
        <>
          ...
          />
          <StatsCard
            title={isRtl ? 'المنتجات التي تحتاج تجديد' : 'Low Stock Items'}
            value={stats.lowStockItems?.toString() || '0'}
            icon={AlertCircle}
            color="red"
            ariaLabel={isRtl ? 'المنتجات التي تحتاج تجديد' : 'Low Stock Items'}
          />
        </>
      )}
  ...
  return (
    <div className={`py-6 px-4 mx-auto max-w-7xl ${isRtl ? 'rtl' : 'ltr'}`}>
