...
interface OrdersVsSalesTableProps {
  data: OrdersVsSalesRow[];
  title: string;
  month: number;
  isRtl: boolean;
  loading: boolean;
...
const OrdersVsSalesTable: React.FC<OrdersVsSalesTableProps> = ({ data, title, isRtl, loading, allBranches, daysInMonth, monthName }) => {
  ...
  const filteredData = useMemo(() => {
    return data
      ...
      .map(row => {
        ...
        let displayedDailyBranchDetailsSales = row.dailyBranchDetailsSales.slice(startDay - 1, endDay);
        if (selectedBranch !== 'all') {
          displayedDailyOrders = displayedDailyOrders.map((_, i) => displayedDailyBranchDetailsOrders[i][selectedBranch] || 0);
          displayedDailySales = displayedDailySales.map((_, i) => displayedDailyBranchDetailsSales[i][selectedBranch] || 0);
        }
        const displayedTotalOrders = displayedDailyOrders.reduce((sum, q) => sum + q, 0);
        const displayedTotalSales = displayedDailySales.reduce((sum, q) => sum + q, 0);
        const displayedTotalRatio = displayedTotalOrders > 0 ? (displayedTotalSales / displayedTotalOrders * 100) : 0;
        return {
          ...row,
          displayedDailyOrders,
          displayedDailySales,
          displayedDailyBranchDetailsOrders,
          displayedDailyBranchDetailsSales,
  ...
  const grandTotalSales = filteredData.reduce((sum, row) => sum + row.displayedTotalSales, 0);
  const grandTotalRatio = grandTotalOrders > 0 ? (grandTotalSales / grandTotalOrders * 100).toFixed(2) : '0.00';

  const getTooltipContent = (dailyQuantity: number, branchDetails: { [branch: string]: number }, isRtl: boolean, type: string) => {
    let header = type === 'orders' ? (isRtl ? 'طلبات' : 'Orders') : (isRtl ? 'مبيعات' : 'Sales');
  ...
  const exportTable = (format: 'excel' | 'pdf') => {
      ...
      })),
      {
        ...
        ...Object.fromEntries(
          displayedDays.flatMap((day, i) => [
            ...
            [`${day} - ${isRtl ? 'مبيعات' : 'Sales'}`, filteredData.reduce((sum, row) => sum + row.displayedDailySales[i], 0)],
            [`${day} - ${isRtl ? 'نسبة %' : 'Ratio %'}`, (filteredData.reduce((sum, row) => sum + row.displayedDailyOrders[i], 0) > 0) ? ((filteredData.reduce((sum, row) => sum + row.displayedDailySales[i], 0) / filteredData.reduce((sum, row) => sum + row.displayedDailyOrders[i], 0)) * 100).toFixed(2) : '0.00'],
          ])
        ),
    ...
    if (format === 'excel') {
      ...
      exportToPDF(dataRows, title, monthName, headers, isRtl, filteredData.length, grandTotalOrders, grandTotalSales, []);
    }
  };
  ...
  return (
    <div className="mb-8">
      ...
      >
        ...
        <Tooltip id="sales-tooltip" place="top" className="custom-tooltip whitespace-pre-line" />
      </motion.div>
    </div>
  );
};

export default OrdersVsSalesTable;