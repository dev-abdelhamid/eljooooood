    ...
    useEffect(() => {
      ...
      const fetchData = async () => {
        ...
        try {
          ...
          if (!ordersResp.success || !Array.isArray(ordersResp.data)) {
            throw new Error('Invalid response format from ordersAPI');
          }
          const branchesResp = await ordersAPI.getBranches(); // افتراض وجود هذه الدالة
          if (!branchesResp.success || !Array.isArray(branchesResp.data)) {
            throw new Error('Invalid response format from branchesAPI');
          }

          const mappedBranches = branchesResp.data.map(b => ({
            _id: b._id,
            name: b.name,
            nameEn: b.nameEn,
          ...
          const combinedData = ordersResp.data.map(order => {
            const totalSales = order.items.reduce((sum, item) => sum + (item.price * item.quantity || 0), 0);
            const dailyProduction = order.items.reduce((sum, item) => sum + (item.status === 'completed' ? item.quantity : 0), 0);

            return {
              orderNumber: order.orderNumber || 'N/A',
              branch: mappedBranches.find(b => b._id === order.branch)?.displayName || 'Unknown',
              product: order.items.map(i => isRtl ? i.product?.name : (i.product?.nameEn || i.product?.name || 'Unknown')).join(', ') || 'N/A',
              quantity: order.items.reduce((sum, i) => sum + (i.quantity || 0), 0),
              status: order.status || 'Unknown',
              date: formatDate(new Date(order.createdAt || Date.now()), language),
