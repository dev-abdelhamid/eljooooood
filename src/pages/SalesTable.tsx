  ...
  const exportTable = (format: 'excel' | 'pdf') => {
    ...
    const rows = [
      ...filteredData.map((row, index) => ({
        ...
        product: row.product,
        unit: row.unit,
        ...Object.fromEntries(filteredDayIndices.map((idx) => [daysInMonth[idx], getDailySale(row, idx)])),
        totalSales: filteredDayIndices.reduce((s, i) => s + getDailySale(row, i), 0),
      ...
      })),
      {
        ...
        unit: '',
        ...Object.fromEntries(filteredDayIndices.map((idx) => [daysInMonth[idx], filteredData.reduce((sum, row) => sum + getDailySale(row, idx), 0)])),
          ...
          <tbody className="divide-y divide-gray-200">
            {filteredData.map((row, index) => (
              <tr key={row.id} className={`hover:bg-blue-50 transition-colors duration-200 ${isRtl ? 'flex-row-reverse' : ''}`}>
                ...
                {filteredDayIndices.map((idx) => {
                  const qty = getDailySale(row, idx);
                  ...
                  return (
                    <td
                      ...
                      data-tooltip-content={getTooltipContent(qty, details, isRtl, 'sales')}
                    >
                      {qty !== 0 ? `${formatNumber(qty, isRtl)}` : '0'}
                    </td>
                  );
                })}
                <td className="px-4 py-3 text-gray-700 text-center font-medium">{formatNumber(filteredDayIndices.reduce((s, i) => s + getDailySale(row, i), 0), isRtl)}</td>
            ...
            <tr className={`font-semibold bg-gray-50 ${isRtl ? 'flex-row-reverse' : ''}`}>
              ...
              {filteredDayIndices.map((idx) => (
                <td key={idx} className="px-4 py-3 text-gray-800 text-center">
                  {formatNumber(filteredData.reduce((sum, row) => sum + getDailySale(row, idx), 0), isRtl)}
                </td>
              ))}
