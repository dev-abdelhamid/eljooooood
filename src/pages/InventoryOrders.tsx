...
export const InventoryOrders: React.FC = () => {
  ...
  const safeTranslate = useCallback((key: string, fallback?: string) => {
    try {
      ...
      return translation !== key ? translation : (fallback || (isRtl ? 'غير مترجم' : 'Not translated'));
    } catch (err) {
      ...
      try {
        ...
        const errorMessage =
          err.response?.status === 404
            ...
            : safeTranslate('error_fetching_orders', { message: err.message }) || (isRtl ? `خطأ في جلب الطلبات: ${err.message}` : `Error fetching orders: ${err.message}`);
  ...
  useEffect(() => {
    ...
    socket.on('newFactoryOrder', (order: any) => {
      ...
      toast.success(safeTranslate('new_order_added', { orderNumber: order.orderNumber }) || (isRtl ? `طلب إنتاج جديد: ${order.orderNumber}` : `New production order: ${order.orderNumber}`), {
    ...
    socket.on('orderStatusUpdated', ({ orderId, status }: { orderId: string; status: FactoryOrder['status'] }) => {
      ...
      toast.info(safeTranslate('order_status_updated', { orderId, status }) || (isRtl ? `تم تحديث حالة الطلب ${orderId} إلى ${status}` : `Order ${orderId} status updated to ${status}`), {
        position: isRtl ? 'top-left' : 'top-right',
      });
    });
    ...
    socket.on('itemStatusUpdated', ({ orderId, itemId, status }: { orderId: string; itemId: string; status: FactoryOrderItem['status'] }) => {
      ...
      toast.info(safeTranslate('item_status_updated', { orderId }) || (isRtl ? `تم تحديث حالة العنصر في الطلب ${orderId}` : `Item status updated in order ${orderId}`), {
        position: isRtl ? 'top-left' : 'top-right',
      });
    });
  ...
  const validateCreateForm = useCallback(() => {
    ...
    state.createFormData.items.forEach((item, index) => {
      if (!item.productId) {
        errors[`item_${index}_productId`] = safeTranslate('product_required', isRtl ? 'المنتج مطلوب' : 'Product is required');
      }
      if (!item.quantity || item.quantity < 1) {
        errors[`item_${index}_quantity`] = item.quantity === 0
          ? safeTranslate('quantity_required', isRtl ? 'الكمية مطلوبة' : 'Quantity is required')
          : safeTranslate('quantity_invalid', isRtl ? 'الكمية يجب أن تكون أكبر من 0' : 'Quantity must be greater than 0');
      }
      if (['admin', 'production'].includes(user.role)) {
        ...
        const availableChefs = departmentId ? state.chefs.filter(chef => chef.department?._id === departmentId && chef.status === 'active') : [];
        if (item.productId && !availableChefs.length && departmentId) {
          errors[`item_${index}_assignedTo`] = safeTranslate('no_chefs_available', isRtl ? 'لا يوجد شيفات متاحة لهذا القسم' : 'No chefs available for this department');
        } else if (!item.assignedTo && availableChefs.length > 0) {
          errors[`item_${index}_assignedTo`] = safeTranslate('chef_required', isRtl ? 'الشيف مطلوب' : 'Chef is required');
        }
      }
    });
  ...
  const createOrder = useCallback(async () => {
    ...
    try {
      ...
      const errorMessage = safeTranslate('error_creating_order', { message: err.message }) || (isRtl ? `فشل في إنشاء الطلب: ${err.message}` : `Failed to create order: ${err.message}`);
  ...
  const updateOrderStatus = useCallback(
    async (orderId: string, newStatus: FactoryOrder['status']) => {
      ...
      try {
        ...
        toast.success(safeTranslate('status_updated', { status: newStatus }) || (isRtl ? `تم تحديث الحالة إلى: ${newStatus}` : `Order status updated to: ${newStatus}`), {
          position: isRtl ? 'top-left' : 'top-right',
        });
      } catch (err: any) {
        console.error('Update order status error:', err.message);
        toast.error(safeTranslate('error_updating_status', { message: err.message }) || (isRtl ? `فشل في تحديث الحالة: ${err.message}` : `Failed to update status: ${err.message}`), {
          position: isRtl ? 'top-left' : 'top-right',
        });
      } finally {
        dispatch({ type: 'SET_SUBMITTING', payload: null });
  ...
  const confirmItemCompletion = useCallback(
    async (orderId: string, itemId: string) => {
      if (!user?.id || user.role !== 'chef') {
        toast.error(safeTranslate('unauthorized_confirm_production', isRtl ? 'غير مصرح بتأكيد الإنتاج' : 'Not authorized to confirm production'), {
          position: isRtl ? 'top-left' : 'top-right',
        });
        return;
      }
      ...
      if (!item || item.assignedTo?._id !== user.id) {
        toast.error(safeTranslate('unauthorized_confirm_item', isRtl ? 'غير مصرح بتأكيد هذا العنصر' : 'Not authorized to confirm this item'), {
          position: isRtl ? 'top-left' : 'top-right',
        });
        return;
      }
      ...
      try {
        ...
        toast.error(safeTranslate('error_confirming_production', { message: err.message }) || (isRtl ? `فشل في تأكيد الإنتاج: ${err.message}` : `Failed to confirm production: ${err.message}`), {
          position: isRtl ? 'top-left' : 'top-right',
        });
      } finally {
        dispatch({ type: 'SET_SUBMITTING', payload: null });
  ...
  const assignChefs = useCallback(
    async (orderId: string) => {
      ...
      try {
        ...
        toast.error(safeTranslate('error_assigning_chefs', { message: err.message }) || (isRtl ? `فشل في تعيين الشيفات: ${err.message}` : `Failed to assign chefs: ${err.message}`), {
          position: isRtl ? 'top-left' : 'top-right',
        });
      } finally {
        dispatch({ type: 'SET_SUBMITTING', payload: null });
          ...
          <Card className="p-6 bg-white shadow-md rounded-xl border border-gray-200">
            ...
            <div className={`flex flex-col sm:flex-row justify-between items-center gap-4 mt-4 ${isRtl ? 'flex-row-reverse' : ''}`}>
              <div className="text-sm text-gray-600 font-medium">
                {safeTranslate('orders_count', { count: filteredOrders.length }) || (isRtl ? `عدد الطلبات: ${filteredOrders.length}` : `Orders count: ${filteredOrders.length}`)}
              </div>
              <Button
                variant="secondary"
              ...
              >
                ...
                {safeTranslate(state.viewMode === 'card' ? 'view_as_table' : 'view_as_cards', state.viewMode === 'card' ? (isRtl ? 'عرض كجدول' : 'View as Table') : (isRtl ? 'عرض كبطاقات' : 'View as Cards'))}
              </Button>
            </div>
          </Card>
                      ...
                      <div>
                        ...
                        {state.createFormData.items.map((item, index) => {
                          ...
                          const availableChefs = departmentId
                            ? state.chefs.filter(chef => chef.department?._id === departmentId && chef.status === 'active')
                            : [];
                          if (item.productId && !availableChefs.length && departmentId && ['admin', 'production'].includes(user.role)) {
                            toast.warn(
                              safeTranslate('no_chefs_available_for_department', { department: selectedProduct?.department?.displayName || 'Unknown' }) ||
                              (isRtl ? `لا يوجد شيفات متاحة لقسم: ${selectedProduct?.department?.displayName || 'غير معروف'}` : `No chefs available for department: ${selectedProduct?.department?.displayName || 'Unknown'}`),
                              { position: isRtl ? 'top-left' : 'top-right' }
                            );
                          }
                          return (
                            <div key={index} className="flex flex-col gap-4 mb-4 p-4 bg-gray-50 rounded-lg border border-gray-100 shadow-sm">
                                ...
                                <div className="flex-1">
                                  <ProductDropdown
                                    ...
                                    onChange={(value) => {
                                      ...
                                      const newAvailableChefs = product?.department?._id
                                        ? state.chefs.filter(chef => chef.department?._id === product.department._id && chef.status === 'active')
                                        : [];
                                      dispatch({
                                        type: 'SET_CREATE_FORM',
                                        payload: {
                                          ...
                                          items: state.createFormData.items.map((it, i) =>
                                            i === index ? { ...it, productId: value, assignedTo: newAvailableChefs.length === 1 ? newAvailableChefs[0].userId : '' } : it
                                          ),
                                        },
                                      });
                                    }}
                                  ...
                                  {state.formErrors[`item_${index}_productId`] && (
                                    <p className="text-red-500 text-xs mt-1">{state.formErrors[`item_${index}_productId`]}</p>
                                  )}
                                </div>
                                <div className="flex-1">
                                  <QuantityInput
                                    ...
                                    onChange={(value) =>
                                      dispatch({
                                        ...
                                        payload: {
                                          ...state.createFormData,
                                          items: state.createFormData.items.map((it, i) =>
                                            i === index ? { ...it, quantity: parseInt(value) || 1 } : it
                                          ),
                                        },
                                      })
                                    }
                                  ...
                                  {state.formErrors[`item_${index}_quantity`] && (
                                    <p className="text-red-500 text-xs mt-1">{state.formErrors[`item_${index}_quantity`]}</p>
                                  )}
                                </div>
                                {['admin', 'production'].includes(user.role) && (
                                  <div className="flex-1">
                                    <ProductDropdown
                                      ...
                                      ariaLabel={safeTranslate('select_chef', isRtl ? 'اختر شيف' : 'Select Chef')}
                                      className={`w-full rounded-lg border-gray-200 focus:ring-amber-500 text-sm shadow-sm transition-all duration-200 ${
                                        state.formErrors[`item_${index}_assignedTo`] ? 'border-red-500' : ''
                                      }`}
                                    />
                                    {state.formErrors[`item_${index}_assignedTo`] && (
                                      <p className="text-red-500 text-xs mt-1">{state.formErrors[`item_${index}_assignedTo`]}</p>
                                    )}
                                  </div>
                                )}
                              </div>
                              {state.createFormData.items.length > 1 && (
                                <Button
                                  ...
                                  className={`flex items-center gap-2 text-red-600 hover:text-red-700 ${isRtl ? 'justify-start' : 'justify-end'} mt-2 text-sm`}
                                  aria-label={safeTranslate('remove_item', isRtl ? 'إزالة العنصر' : 'Remove Item')}
                                >
                                  <MinusCircle className="w-4 h-4" />
                                  {safeTranslate('remove_item', isRtl ? 'إزالة العنصر' : 'Remove Item')}
                                </Button>
                              )}
                            </div>
                          );
                        ...
                        <Button
                          ...
                          className="flex items-center gap-2 bg-amber-100 hover:bg-amber-200 text-amber-800 rounded-lg px-4 py-2 text-sm font-medium shadow transition-all duration-200 w-full sm:w-auto"
                          aria-label={safeTranslate('add_item', isRtl ? 'إضافة عنصر' : 'Add Item')}
                        >
  ...
  {safeTranslate('add_item', isRtl ? 'إضافة عنصر' : 'Add Item')}
                      ...
                      {state.formErrors.form && (
                        <div className="mt-4 p-3 bg-red-50 text-red-600 text-sm rounded-lg flex items-center gap-2">
                          ...
                          <p>{state.formErrors.form}</p>
                        </div>
                      )}
                      <div className={`flex gap-4 mt-6 ${isRtl ? 'justify-start flex-row-reverse' : 'justify-end'}`}>
                        <Button
                          variant="secondary"
                          onClick={() => {
                            dispatch({ type: 'SET_CREATE_MODAL', isOpen: false });
                          ...
                          aria-label={safeTranslate('cancel', isRtl ? 'إلغاء' : 'Cancel')}
                        ...
                        <Button
                          variant="primary"
                          type="submit"
                          className="bg-amber-600 hover:bg-amber-700 text-white rounded-lg px-4 py-2 text-sm font-medium shadow transition-all duration-200"
                          aria-label={safeTranslate('create_order', isRtl ? 'إنشاء الطلب' : 'Create Order')}
                          disabled={state.submitting === 'create'}
                        >
                          {state.submitting === 'create' ? (
                            <div className="flex items-center gap-2">
                              ...
                              {safeTranslate('creating', isRtl ? 'جاري الإنشاء...' : 'Creating...')}
                            </div>
                          ) : (
