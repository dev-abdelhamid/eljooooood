...
const OrdersTable: React.FC<Props> = ({
  ...
  const getDailyQuantity = (row: OrderRow, i: number) => {
  ...
  const exportTable = (format: 'excel' | 'pdf') => {
    ...
    if (format === 'excel') {
      ...
      ws['!cols'] = [{ wch: 10 }, { wch: 15 }, { wch: 20 }, { wch: 15 }, ...filteredDayIndices.map(() => ({ wch: 12 })), { wch: 15 }, { wch: 15 }, { wch: 15 }, { wch: 15 }];
                ...
                {filteredDayIndices.map((idx) => {
                  ...
                  return (
                    <td
                      ...
                      data-tooltip-content={getTooltipContent(qty, details, isRtl, 'orders')}
                    >
