...
// Convert numbers to Arabic numerals, including decimals
...
const generatePDFHeader = (
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    const footerText = isRtl
      ? doc.processArabic(`تم إنشاؤه بواسطة نظام إدارة الجودياء - ${toArabicNumerals(currentDate)}`)
...
const generatePDFTable = (
  ...
  const processedHeaders = isRtl ? headers.map(header => doc.processArabic(header)).reverse() : headers;
  const processedData = data.map(row => isRtl ? row.map((cell: string) => doc.processArabic(cell)).reverse() : row);
  ...
  try {
    ...
    const filteredOrders = orders.filter(
      (order) =>
        ...
        !isNaN(order.adjustedTotal)
    );

    if (filteredOrders.length === 0) {
      toast.warn(isRtl ? 'لا توجد طلبات صالحة للتصدير' : 'No valid orders to export', {
        position: isRtl ? 'top-left' : 'top-right',
        autoClose: 3000,
      });
      return;
    ...
    const data = filteredOrders.map((order) => {
      ...
      const numericAmount = parseFloat(totalAmountStr.replace(/[^0-9.,]/g, '').replace(',', '.'));
      const formattedTotalAmount = !isNaN(numericAmount) ? formatPrice(numericAmount, isRtl) : formatPrice(0, isRtl);
      return [
