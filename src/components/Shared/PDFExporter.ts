...
// Format price with conditional commas
const formatPrice = (amount: number | undefined, isRtl: boolean): string => {
  ...
  const formatted = validAmount >= 1000
    ? validAmount.toLocaleString(isRtl ? 'ar-SA' : 'en-US', { minimumFractionDigits: 0, maximumFractionDigits: 0 })
    : String(validAmount); // No commas for numbers < 1000
...
const formatProducts = (items: Order['items'], isRtl: boolean, translateUnit: (unit: string, isRtl: boolean) => string): string => {
  return items
    .map((item) => {
      ...
      const name = isRtl && item.productNameEn ? item.productNameEn : item.productName;
