...
// Load Amiri font (regular and bold) for reliable Arabic rendering
...
const generatePDFHeader = (
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    const footerText = isRtl
      ? doc.processArabic(`تم إنشاؤه بواسطة نظام إدارة الجودياء  `)
...
const generatePDFTable = (
  ...
  fontLoaded: boolean,
  fontName: string,
  calculateAdjustedTotal: (order: Order, isRtl: boolean) => string,
...
const calculateAdjustedTotal = (order: Order, isRtl: boolean): string => {
  console.log(`Processing order ${order.orderNumber}:`, order.items); // Log order items for debugging
  try {
    if (!order.items || !Array.isArray(order.items) || order.items.length === 0) {
      console.warn(`Order ${order.orderNumber} has no valid items`);
      return formatPrice(0, isRtl);
    }
    const total = order.items.reduce((sum, item) => {
      const price = (typeof item.price === 'number' && !isNaN(item.price)) ? item.price : 0;
      const quantity = (typeof item.quantity === 'number' && !isNaN(item.quantity)) ? item.quantity : 0;
      console.log(`Item: ${item.productName}, Price: ${price}, Quantity: ${quantity}`); // Log item details
      return sum + (price * quantity);
    }, 0);
    const formattedTotal = formatPrice(total, isRtl);
    console.log(`Order ${order.orderNumber} total: ${formattedTotal}`); // Log final total
    return formattedTotal;
  } catch (error) {
    console.error(`Error calculating total for order ${order.orderNumber}:`, error);
    return formatPrice(0, isRtl);
  }
};
...
export const exportToPDF = async (
  ...
  try {
    console.log('Input orders:', orders); // Log input orders for debugging
    ...
    if (filteredOrders.length === 0) {
      console.warn('No orders to export after filtering');
      toast.warn(isRtl ? 'لا توجد طلبات لتصديرها' : 'No orders to export', {
    ...
    const totalAmount = filteredOrders.reduce((sum, order) => {
      const amountStr = calculateAdjustedTotal(order, isRtl).replace(/[^0-9.,]/g, '').replace(',', '.');
      const amount = amountStr && !isNaN(parseFloat(amountStr)) ? parseFloat(amountStr) : 0;
      console.log(`Order ${order.orderNumber} amount: ${amount}`); // Log amount for each order
      return sum + amount;
    }, 0);
    ...
    const data = filteredOrders.map((order) => {
      ...
      const totalAmountStr = calculateAdjustedTotal(order, isRtl);
      return [
