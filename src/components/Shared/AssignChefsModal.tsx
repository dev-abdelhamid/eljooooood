...
const AssignChefsModal: React.FC<AssignChefsModalProps> = memo(
  ({
    isOpen,
    onClose,
    selectedOrder,
    ...
    return (
      ...
      >
        <form
          ...
          className="space-y-6 max-h-96 overflow-y-auto p-2"
        >
          {assignFormData.items.map((item, index) => {
            ...
            return (
              ...
              >
                ...
                >
                  {isRtl
                    ? `تعيين شيف لـ ${orderItem?.displayProductName} (${item.quantity} ${orderItem?.displayUnit})`
                    : `Assign chef to ${orderItem?.displayProductName} (${item.quantity} ${orderItem?.displayUnit})`}
                </label>
                <Select
          ...
          {availableChefsByDepartment.size === 0 && (
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
            ...
            >
              ...
              <span className="text-red-600 text-sm">{isRtl ? 'لا يوجد شيفات متاحة' : 'No chefs available'}</span>
            </motion.div>
          )}
          {error && (
            <motion.div
          ...
          <div className={`flex justify-end gap-2 ${isRtl ? 'flex-row-reverse' : ''}`}>
            <Button
              variant="secondary"
              onClick={onClose}
              className="bg-gray-200 hover:bg-gray-300 text-gray-800 rounded-md px-4 py-2 text-sm shadow-sm transition-all"
            ...
            <Button
              ...
              className="bg-amber-500 hover:bg-amber-600 text-white rounded-md px-4 py-2 text-sm shadow-sm transition-all"
