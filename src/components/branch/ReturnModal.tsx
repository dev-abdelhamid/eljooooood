          ...
          <div>
            ...
            <Select
              options={order.items.map(item => ({
                value: item.itemId,
                label: isRtl ? `${item.productName} (${item.quantity} ${t(`${item.unit || 'unit'}`)})` : `${item.productName} (${item.quantity} ${t(`${item.unit || 'unit'}`)})`,
              }))}
          ...
          <div>
            ...
            <Select
              options={returnReasonOptions.map(opt => ({
                value: opt.value,
                label: isRtl ? t(`orders.return_reasons_${opt.label}`) : t(`orders.return_reasons_${opt.label}`),
              }))}
