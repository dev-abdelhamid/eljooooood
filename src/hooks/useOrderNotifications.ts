        ...
        handler: async (data: any) => {
          ...
          const mappedOrder: Order = {
            ...
            items: data.items.map((item: any) => ({
              ...
              productNameEn: item.product?.nameEn || t('products.unknown'),
              quantity: Number(item.quantity) || 1,
              price: Number(item.price) || 0,
              unit: translateUnit(item.unit || item.product?.unit),
              unitEn: translateUnit(item.unit || item.product?.unitEn),
              department: item.product?.department || { _id: 'unknown', name: t('departments.unknown')  , nameEn: t('departments.unknown') },
